...
class OBJViewerState extends State<OBJViewer> {
  ...
  void _updateTransform() {
    ...
    final viewMatrix = vec32.makeViewMatrix(vec32.Vector3(0.0, 0.0, 4.0), vec32.Vector3(0.0, 0.0, 0.0), vec32.Vector3(0.0, 1.0, 0.0));
...
class OBJLoader {
  ...
  void _parseMTLFile() {
    ...
    for (var line in mtlLines) {
      ...
      if (line.startsWith('newmtl ')) {
        ...
        final args = line.split(' ');
        currentMaterial.diffuseColor = Color.fromARGB(255, (double.parse(args[1]) * 255).round(), (double.parse(args[2]) * 255).round(), (double.parse(args[3]) * 255).round());
  ...
  Future<void> _loadMTLTextures() async {
    ...
    for (var mtl in _materials.values) {
      ...
      AssetImage(path.join(_basePath, mtl.texturePath), bundle: _bundle).resolve(const ImageConfiguration()).addListener(
...
class VertexMeshInstance {
  ...
  void _cacheVertices() {
    ...
    List<vec32.Vector4> transformedPositions = <vec32.Vector4>[];
    ...
    for (int i = 0; i < _mesh.vertexCount; ++i) {
      vec32.Vector4 position = vec32.Vector4(_mesh.positions[i * 3 + 0], _mesh.positions[i * 3 + 1], _mesh.positions[i * 3 + 2], 1.0);
    ...
    for (int i = 0; i < colors.length; ++i) {
      ...
      colors[i] = 0xFF000000 | ((b * ((_mesh.colors[i] >> 16) & 0xFF)).floor() << 16) | ((b * ((_mesh.colors[i] >> 8) & 0xFF)).floor() << 8) | ((b * ((_mesh.colors[i] >> 0) & 0xFF)).floor() << 0);
    }

    _vertices = ui.Vertices.raw(VertexMode.triangles, positions2D, indices: Uint16List.fromList(culledIndices), textureCoordinates: _mesh.uvs, colors: colors);
...
class MeshCustomPainter extends CustomPainter {
  ...
  void paint(canvas, size) {
    ...
    final paint = Paint();
    paint.shader = ImageShader(_meshInstance.texture, TileMode.clamp, TileMode.clamp, Matrix4.identity().scaled(1 / _meshInstance.texture.width, 1 / _meshInstance.texture.height, 1.0).storage);
